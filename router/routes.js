...
router.post('/login', async (req, res) => {  //route for login
    ...
    try {
        ...
        if (username=='admin' && password=='admin123') {
            ...
            const token = jwt.sign({ username, _id:DB.User._id }, `${process.env.SECRET_KEY}`, { expiresIn: '5h' });
            console.log( 'secret-key: '+`${process.env.SECRET_KEY}`+' Admin token: '+ token )
        } 
          ...
          if(user && (user.isDisabled==false) )
            {
              ...
              const token = jwt.sign({ username, _id: DB.User._id }, `${process.env.SECRET_KEY}`, { expiresIn: '5h' });
              console.log( 'secret-key: '+`${process.env.SECRET_KEY}`+' token: '+ token )
...
router.get('/profile/DisplayBlogs', async (req, res) => {
  try {
    ...
    const totalBlogs = await DB.BlogPost.countDocuments({ disabled: { $ne: true } });
